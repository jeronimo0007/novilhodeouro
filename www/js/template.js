	...
	html+='<div class="wrap">';	
	  ...
	  html+='<p class="description">'+data.item_description+'</p>';
       ...
       if(data.gallery.length>0){
       	  ...
       	  html+='<div class="photo_gallery" style="padding:0;margin:0 0 10px;" >';       	  
...
var displayCartDetails = function(datas){
				...
				$.each( item_val.new_sub_item, function( new_sub_item_key, new_sub_item_val ) {
					html+='<ons-list-item modifier="nodivider normal_list" >';
					    ...
					    $.each( new_sub_item_val , function( new_sub_item_val_key, new_sub_item_val_val ) {
					    	...
					    	html+='<ons-row>';
					    	  html+='<ons-col vertical-align="center" width="100px" >'+ new_sub_item_val_val.addon_qty + 'x' + prettyPrice(new_sub_item_val_val.addon_price)  +'</ons-col>';
					    	  html+='<ons-col vertical-align="center" >'+ new_sub_item_val_val.addon_name  +'</ons-col>';
					    	  html+='<ons-col vertical-align="center" class="text_right" width="100px" >'+  prettyPrice(parseFloat(new_sub_item_val_val.addon_qty)*parseFloat(new_sub_item_val_val.addon_price))  +'</ons-col>';
					    	html+='</ons-row>';
					    });
					html+='</ons-list-item>';
				});
	...
	/*TIPS*/
	if(has_tips==false && datas.transaction_type=="delivery" ){	
...
setBookingList = function(element, data){
	...
	$.each( data  , function( key, val ) {
		...
		params = clickFormat(val.booking_id+"|"+val.can_cancel);
		 html+='<ons-list-item modifier="list_style_banner nodivider" tappable onclick="actionSheetBooking('+ params +')"   >';
           html+='<div class="inner raty-medium">';
              html+='<ons-row>';
		...
		if(social_button_enabled>1){			
	        ...
	        html+='<ons-list-item modifier="nodivider">';
			  html+='<ons-row>';
			    ...
			    if(settings.singleapp_enabled_fblogin==1){
			    html+='<ons-col vertical-align="center"  > ';
			       html+='<ons-button modifier="large blue_button no_shadow" class="full_width" onclick="fbLogin()" >';
