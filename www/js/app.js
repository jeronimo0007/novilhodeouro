...
var device_id  = 'deviceid_1234567890';
...
var interval_timeout = 30000;
var track_interval;

var code_version = 2.3 // do not change this value;
...
document.addEventListener('init', function(event) {
   ...
   switch (page_id)
   {
   	  ...
   	  case "forgot_pass":
   	    $(".tabbar--forgot_tab1__button .tabbar__label").html( t("VIA EMAIL") );
   	    $(".tabbar--forgot_tab2__button .tabbar__label").html( t("VIA SMS") );
		...
		tabbar_loaded = true;
   	  ...
   	  case "payment_option":
		  ...
		  if(!empty(opt_contact_delivery)){
   	      ...
   	      ajaxCall('loadPaymentList', params );    	    
   	  break;
   	  ...
   	  case "dinein_forms":
   	     $(".pay_now_label").html( t("PAY")+ " " + $(".cart_total_value").val() );
   	      ...
   	      $("[name='dinein_number_of_guest']").attr("placeholder", t("Number of guest") );   	     
   	      $("[name='dinein_special_instruction']").attr("placeholder", t("Special instructions") ); 
   	      $("[name='dinein_table_number']").attr("placeholder", t("Table number") ); 
   	      $("[name='contact_phone']").attr("placeholder", t("Mobile number") ); 
        ...
        $(".device_id").html( device_id );
   	    ...
   	    if(isLogin()){   	    
   	    	$(".show_if_login").show();    	    	
   	    	$alert_enabled_push = parseInt($(".alert_enabled_push").val());
   	    	$alert_enabled_subcribe = parseInt($(".alert_enabled_subcribe").val());
   	    	if($alert_enabled_push==1){
   	    		$(".enabled_push").prop('checked', true);
   	    	}
   	    	if($alert_enabled_subcribe==1){
   	 ...
   	 case "contact_us_ty":
   	  ...
   	  case "kushi_form":   	
   	    ...
   	    $("#kushi_form .order_id").val( page.data.order_id );
   	    
   	    placeholder(".card_name",'First Name');
   	    placeholder(".card_last_name",'Last Name');
   	    placeholder(".credit_card_number",'Credit Card Number');
   	    placeholder(".billing_address",'Billing address');
   	    placeholder(".cvv",'CVV');
   	  ...
   	  case "kushi_form_charge":   	    
   	    $("#kushi_form_charge .transaction_token").val( page.data.transaction_token );
   	    $("#kushi_form_charge .reference_id").val( page.data.reference_id );
   	    
   	    kushiPaymentDetails('kushi_list',{
   	    	card_details : page.data.card_details,
   	        payment_description : page.data.payment_description,
	        ...
	        switch (action)
	        {
	        	...
	        	case "GetAddressFromCart":
	        	  ...
	        	  if(empty(data.details.contact_phone)){
	        	  	customer_number = getStorage("customer_number");   	    
				   	if(!empty(customer_number)){	   	   
				   	   $("#address_form .contact_phone").val( customer_number );
				   	}   	    	 
	        	  }	        
	        	...
	        	case "getUserProfile":
	        	  ...
	        	  $(".alert_enabled_push").val( data.details.data.enabled_push );
	        	  $(".alert_enabled_subcribe").val( data.details.data.subscribe_topic );
	        	...
	        	case "savePushSettings":	        	  
	        	  $(".alert_enabled_push").val( data.details.enabled_push );
	        	  showToast(data.msg);
	        	break;
	        	...
	        	case "ReOrder":	        		        	  
	        	  if ((typeof data.details !== "undefined") && ( data.details !== null)) {	        	  	  
	        	  	  setStorage("transaction_type", data.details);
	        	  }
	        	  showCart();
	        	break;
	        	...
	        	case "KushiRequestToken":
	        	  showPage('kushi_form_charge.html','none',{
	        	  	'transaction_token': data.details.transaction_token,
	        	  	'reference_id': data.details.reference_id,
	        	  	'payment_description': data.details.payment_description,
	        	  	'amount': data.details.amount,
	        	  	'card_details': data.details.card_details,
	        	  });
	        	break;
	        	...
	        	case "cancelBooking":
			      ...
			      processDynamicAjax('loadBooking',params,'booking_loader','GET',1 ); 
	        	break;
	        	
	        	case "saveSubsribe":
	        	  $(".alert_enabled_subcribe").val( data.details.subscribe_topic );
	        	  showToast(data.msg);
	        	break;
...
var receivePush = function(){	
	var enabled_push = parseInt($("input[name=enabled_push]:checked").val());	
	if (isNaN(enabled_push)){
		enabled_push=0;
	}
...
payNowNextStep = function(data){
	...
	switch (data.details.next_step){
		...
		case "init_kushi":
		  onsenNavigator.pushPage('kushi_form.html',{
			  	animation : "slide",
			  	data : {
			  		order_id : data.details.order_id
			  	}
        		...
        		case "getlanguageList":        		  
        	 	...
        	 	case "mapboxgeocode":
        	 	  ...
        	 	  $(".identify_location_wrap").hide();
			   	  ...
			   	  current_page_id = onsenNavigator.topPage.id;	
			   	  $("#"+current_page_id +" .search_address2").val( $resp.formatted_address );   	  	  	 			   	  
	    ...
	    buttons: action_sheet
	  	  ...
	  	  switch(index){
			    ...
			    ons.notification.confirm( t("Are you sure?") ,{
					...
					if (input==0){			
						ajaxCall('cancelBooking', 'booking_id=' + booking_id );
					}
				});
	  	  	break;	  	  		  	  
	  	  }
	...
	try {
		...
		if(subscribe_topic>0){			
			window.FirebasePlugin.subscribe("broadcast", function(){		        
		        ajaxCall('saveSubsribe', "subscribe_topic="+subscribe_topic );
		    },function(error){
...
initAndroid = function(){
	var customChannel  = {
	 ...
	 window.FirebasePlugin.setDefaultChannel(customChannel,
        function() {                        
        	showToast("setDefaultChannel created", error);
        },
        function(error) {
...
function subscribe(){
	try {
		...
		$topics = getStorage("topics");				
	    window.FirebasePlugin.subscribe($topics, function(){
...
function unsubscribe(){
	try {
		$topics = getStorage("topics");
	    window.FirebasePlugin.unsubscribe($topics, function(){
